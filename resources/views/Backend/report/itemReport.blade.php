        ...
        <div class="row date_button">
            <div class="col-md-11 date_row">
                ...
                <div class="col-md-12 btn-gp">
                    ...
                    {!! Form::open(array('url' => 'Backend/itemReportWithDate', 'method' => 'post', 'class'=> 'form-horizontal user-form-border')) !!}
                    <div class="col-md-3">
                    ...
                    <div class="col-md-1 pull-right ">
                        @if(isset($start_date))
                            @if($number=="" && $from_amount == "")
                                <a href={{'/Backend/downloadItemReportWithDate/'.$start_date.'/'.$end_date}}><button class="btn btn-primary ">Export</button></a>
                            @elseif($number != "" && $from_amount == "")
                                <a href={{'/Backend/downloadItemReportWithDate/'.$start_date.'/'.$end_date.'/'.$number}}><button class="btn btn-primary ">Export</button></a>
                            @elseif($number == "" && $from_amount != "")
                                <a href={{'/Backend/downloadItemReportWithDate/'.$start_date.'/'.$end_date.'/'.$from_amount.'/'.$to_amount}}><button class="btn btn-primary ">Export</button></a>
                            @else
                                <a href={{'/Backend/downloadItemReportWithDate/'.$start_date.'/'.$end_date.'/'.$number.'/'.$from_amount.'/'.$to_amount}}><button class="btn btn-primary ">Export</button></a>
                            @endif
                        @else
                            <a href={{'/Backend/downloadItemReport'}}><button class="btn btn-primary ">Export</button></a>
                        @endif
                    </div>
                </div>
            </div>
